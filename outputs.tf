...
output "postgresql_for_hasura_database_url" {
  ...
  sensitive = true
  value = "postgresql://${var.database_username}:${var.database_password}@${module.aws_rds_cluster.cluster.endpoint}:5432/${var.database_name}"
}
...
output "postgresql_for_hasura_passwordless_database_url" {
  ...
  sensitive = false
  value = "postgresql://${var.database_username}:REPLACE_WITH_PASSWORD@${module.aws_rds_cluster.cluster.endpoint}:5432/${var.database_name}"
}
